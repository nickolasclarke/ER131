...
def run_plotly(observed_data, predicted_data, total_data, hour, k):
    ...
    predicted = go.Scattermapbox(
        ...
        marker=dict(
            ...
            opacity=0.8,
            cmin = 0,
            color = predicted_data['Log Sample Measurement'],
            cmax = total_data['Log Sample Measurement'].max(),
            colorscale = [
                [0, 'rgb(34,139,34)'],
                [0.5, 'rgb(204,204,0)'],
                [1, 'rgb(178,34,34)']
    ...
    observed = go.Scattermapbox(
        ...
        marker=dict(
            ...
            opacity=0.8,
            cmin = 0,
            color = observed_data['Log Sample Measurement'],
            cmax = total_data['Log Sample Measurement'].max(),
            colorscale = [
                [0, 'rgb(34,139,34)'],
                [0.5, 'rgb(204,204,0)'],
    ...
    layout = go.Layout(
            title='PM2.5 Concentration on Nov 9, 2018 at ' + hour + ' with K = ' + str(k),
            autosize=True,
            hovermode='closest',
            mapbox=dict(
